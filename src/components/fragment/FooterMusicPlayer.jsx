...
function FooterMusicPlayer({ music }) {
  let currentIndex;
  const [{ name, image, audio }, setCurrTrack] = useState(music);
  ...
  const { playlists, playing } = useSelector(state => state.musicReducer);
  ...
  console.log(playing);
  console.log(
    _.findIndex(playlists, function (o) {
      return o._id === playing._id;
    })
  );
  currentIndex = _.findIndex(playlists, function (o) {
    return o._id === playing._id;
  });
  ...
  useEffect(() => {
    if (isNextClicked) {
      let currTrackId = (currentIndex + 1) % playlists.length;
  ...
  }, [dispatch, currentIndex, isNextClicked, isPrevClicked, playlists]);
  ...
  return (
    <div style={useStyle.component} className={'footer-player'}>
      ...
      <Button
        startIcon={<Avatar variant="square" src={image} alt={name} />}
      ...
      <div className="playback-controls">
        ...
        <audio ref={audioElement} src={audio} preload={'metadata'} />
